...
void Tracking::PollCurrentFrame() {
    mHasNextFrame.wait();
    mCurrentFrame = std::move(mNextFrame);
    Track();
    mLastProcessedFrame = mCurrentFrame;
}
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
   ...
   // else
    ...
    if (mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        mNextFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    else
        mNextFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    mHasNextFrame.notify();
